...
async function fetchRooms(start_date, end_date) {
  ...
  const sceleton = sceletonTemplate();
  const section_rooms = document.getElementById('section--rooms');
  section_rooms.innerHTML = sceleton;
...
let amenitie = '';
    if(room._embedded.amenities){
         amenitie = room._embedded.amenities.reduce((acc, item) => {
            return acc + `<div>${item.description}</div>`;
        }, '');
    }
  ...
  if (index === 0) {
    ...
    first_room_box.getElementsByClassName("room-card--left")[0].innerHTML = `
        ...
        />
    `;
    first_room_box.getElementsByClassName("room-card--header")[0].innerText = room.description;
    first_room_box.getElementsByClassName("room-card--body--amenities")[0].innerHTML = amenitie;
    first_room_box.getElementsByClassName("room-card--body--price--number")[0].innerText = room.full_formatted_price;
    first_room_box.getElementsByClassName("room-card--body--price--nights")[0].innerText = `for ${room.fullPriceBreakdown.nights} nights`;
    first_room_box.getElementsByClassName("room-card--body--price--number")[0].innerText = room.full_formatted_price;

  } else {
    return `<article class="room-card">
                <div class="room-card--left">
                    <img src="${room._embedded.pictures[0].offer_teaser_ncol}" alt="${room._embedded.pictures[0].description}" loading="lazy" />
                </div>
      ...
      <div id="section--rooms--1" class="room-card suspense-block">
                        ...
                        <div class="room-card--right">
                            <div class="room-card--body">
                                ...
                                <div class="room-card--body--amenities suspense-line">
                                </div>
                            </div>
